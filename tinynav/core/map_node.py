...
class MapNode(Node):
    ...
    def relocalize_with_depth(self, keyframe: np.ndarray, keyframe_features: dict, K: np.ndarray | None) -> tuple[bool, np.ndarray, float]:
        ...
        # Use VLAD retrieval if available, otherwise fall back to CLS-token.
        if self.vlad_centres is not None and self.map_vlad_descriptors is not None:
            query_vlad = self.get_vlad_descriptor(keyframe)
            if query_vlad is None:
                return False, np.eye(4), -np.inf
            idx_and_similarity_array = find_loop_vlad(
                query_vlad, self.map_vlad_descriptors,
                self.relocalization_threshold, self.relocalization_loop_top_k,
            )
            max_similarity = (
                max((s for _, s in idx_and_similarity_array), default=0.0)
                if idx_and_similarity_array else 0.0
            )
            if not idx_and_similarity_array:
