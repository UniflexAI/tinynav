...
# target pose validation/safety params
TARGET_STEPS = 8
TARGET_SAMPLE_RADII = [0.08, 0.16, 0.24, 0.35, 0.45, 0.60, 0.75]
TARGET_Z_OFFSET_CANDIDATES = [-0.05, 0.0, 0.05]
TARGET_SAFETY_MARGIN = 0.2  # meters
TARGET_OBS_OCC_THRESHOLD = 0.05
...
class PlanningNode(Node):
    ...
    def world_to_grid(self, point):
        """Convert world xyz point to integer voxel indices.

        Returns (idx, valid) where valid indicates all indices are in bounds.
        """
        point = np.asarray(point)
        idx = np.floor((point - self.origin) / self.resolution).astype(int)
        valid = (
            0 <= idx[0] < self.occupancy_grid.shape[0]
            and 0 <= idx[1] < self.occupancy_grid.shape[1]
            and 0 <= idx[2] < self.occupancy_grid.shape[2]
        )
        return idx, valid

    def is_target_pose_safe(self, target_pose, ESDF_map):
        """Return True if target is NOT confirmed to be inside/near an obstacle.

        Principle: only block when we have positive evidence of danger.
        Unknown/unobserved areas are treated as safe — the local planner
        should not refuse to move just because it hasn't looked somewhere yet.
        """
        if target_pose is None:
            return False

        idx, valid = self.world_to_grid(target_pose)
        if not valid:
            return True  # outside local map -> not our safety concern

        i, j, k = int(idx[0]), int(idx[1]), int(idx[2])

        # ESDF clearance check: too close to known obstacle
        if ESDF_map is not None:
            esdf_val = ESDF_map[i, j]
            if np.isfinite(esdf_val) and esdf_val < TARGET_SAFETY_MARGIN:
                return False

        # Footprint collision check in 3D occupancy grid:
        # only reject if we see confirmed occupied voxels nearby
        _, _, half_w = self.robot.footprint_from_control()
        radius_vox = max(1, int(np.ceil(half_w / self.resolution)))
        k_min = max(0, k - 1)
        k_max = min(self.occupancy_grid.shape[2] - 1, k + 1)
        for di in range(-radius_vox, radius_vox + 1):
            ii = i + di
            if ii < 0 or ii >= self.occupancy_grid.shape[0]:
                continue
            for dj in range(-radius_vox, radius_vox + 1):
                jj = j + dj
                if jj < 0 or jj >= self.occupancy_grid.shape[1]:
                    continue
                col = self.occupancy_grid[ii, jj, k_min : k_max + 1]
                if np.any(col > TARGET_OBS_OCC_THRESHOLD):
                    return False
        return True

    def choose_safe_target_pose(self, target_pose, ESDF_map):
        """If target is unsafe, sample nearby points and find the nearest safe one.
        ...
        base = np.asarray(target_pose)
        # strict check for original target
        if self.is_target_pose_safe(base, ESDF_map):
            return base

        # sample around target, nearest-first strategy by radius list
        best = None
        best_dist = 1e9
        for r in TARGET_SAMPLE_RADII:
            for step in range(TARGET_STEPS):
                angle = 2 * np.pi * step / TARGET_STEPS
                xy_offset = np.array([r * np.cos(angle), r * np.sin(angle), 0.0])
                for z_off in TARGET_Z_OFFSET_CANDIDATES:
                    cand = base + xy_offset + np.array([0.0, 0.0, z_off])
                    if self.is_target_pose_safe(cand, ESDF_map):
                        d = np.linalg.norm(cand - base)
                        if d < best_dist:
                            best_dist = d
                            best = cand
        return best
    ...
    def sync_callback(self, depth_msg, odom_msg):
        ...
        with Timer(name='traj score', text="[{name}] Elapsed time: {milliseconds:.0f} ms"):
            ...
            scores, occ_points = score_trajectories_by_ESDF(trajectories, ESDF_map, self.origin, self.resolution, self.robot.safety_radius, front_len, rear_len, 0.3)
            top_k = 100
        ...
        with Timer(name='pub', text="[{name}] Elapsed time: {milliseconds:.0f} ms"):
            ...
            def cost_function(traj, param, score, target_pose):
                ...
                # regular trajectory penalty - use safe_target_pose
                traj_end = np.array(traj[-1,:3])
                safe_target = self.safe_target_pose if self.target_pose is not None else None
                target_end = safe_target if safe_target is not None else traj_end
