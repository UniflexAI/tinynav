...
def snap_target_to_free(target_xy, robot_xy, forward_xy, obstacle_mask, origin, resolution,
                        search_radius=10, heading_weight=0.3):
    """When target is in obstacle, find nearby free cell preferring robot's current heading."""
    h, w = obstacle_mask.shape
    ...
    for r in range(1, search_radius + 1):
        ...
        for x in range(x0, x1 + 1):
            for y in range(y0, y1 + 1):
                ...
                score = dist + heading_weight * search_radius * heading_penalty
                if score < best_score:
                    best_score = score
                    best = cell_world
        if best is not None:
            break
    return best if best is not None else target_xy
...
def plan_dwa_minimal(robot_xy, robot_forward_xy, target_xy, obstacle_mask, cost_map, origin, resolution,
    ...
    if v_samples is None:
        v_samples = np.array([-0.1, 0.00, 0.1, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40], dtype=np.float32)
    if w_samples is None:
        w_samples = np.array([-1.0, -0.7, -0.5, -0.3, -0.15, 0.0, 0.15, 0.3, 0.5, 0.7, 1.0], dtype=np.float32)
...
class PlanningNode(Node):
    def __init__(self, sensor_source: str = 'auto', robot_config_path: str = None):
        ...
        # Static wall memory:
        # - FOV seen & no wall => fast clear
        # - FOV unseen => slow decay keep
        self.wall_conf = np.zeros(self.grid_shape[:2], dtype=np.float32)
        self.wall_conf_decay_seen = 0.55
        self.wall_conf_decay_unseen = 0.985
        self.wall_conf_threshold = 0.35
        self.static_wall_occ_threshold = 0.08
        self.static_wall_span_m = 0.7
        self.K = None
        self.baseline = None
        self.last_T = None
        ...
        self.max_angular_acc = 0.8   # rad/s^2 (unlock turning response)
    ...
    def sync_callback(self, depth_msg, odom_msg):
        ...
        with Timer(name='heightmap+obstacle', text="[{name}] Elapsed time: {milliseconds:.0f} ms"):
            ...
            static_wall_now = build_static_wall_mask(
                self.occupancy_grid,
                self.resolution,
                occ_threshold=self.static_wall_occ_threshold,
                min_wall_span_m=self.static_wall_span_m,
            )
            seen_now = np.any(new_occ != 0.0, axis=2)
            unseen = ~seen_now
            self.wall_conf[unseen] *= self.wall_conf_decay_unseen
            clear_mask = seen_now & (~static_wall_now)
            self.wall_conf[clear_mask] *= self.wall_conf_decay_seen
            self.wall_conf[static_wall_now] = 1.0
            static_wall_keep = self.wall_conf >= self.wall_conf_threshold
            obstacle_mask = obstacle_mask | static_wall_keep
