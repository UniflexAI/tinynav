...
using Observation2D = std::tuple<int64_t, int64_t, py::array_t<double>>;     // (cam_idx, pt_idx, 2x1)
using Observations2D = std::vector<Observation2D>;
using Observation3DWeighted = std::tuple<int64_t, int64_t, py::array_t<double>, py::array_t<double>>;
using Observations3DWeighted = std::vector<Observation3DWeighted>;
...
class MahalanobisPointToCameraError {
public:
    MahalanobisPointToCameraError(Eigen::Vector3d observed_point_in_camera, Eigen::Matrix3d sqrt_information)
        : observed_point_in_camera_(observed_point_in_camera), sqrt_information_(sqrt_information) {}
    template<typename T>
    bool operator()(const T* camera_parameters, const T* point_parameters, T* residuals) const {
        using Vector3T = Eigen::Matrix<T, 3, 1>;
        Eigen::Map<const Eigen::Matrix<T, 6, 1>> camera(camera_parameters);
        ...
        ceres::AngleAxisToRotationMatrix(phi.data(), rotation.data());
        Vector3T point_3d_in_camera = rotation.transpose() * (point_3d - translation);
        Eigen::Matrix<T, 3, 1> delta = point_3d_in_camera - observed_point_in_camera_.cast<T>();
        Eigen::Matrix<T, 3, 1> weighted = sqrt_information_.cast<T>() * delta;
    ...
    private:
    Eigen::Vector3d observed_point_in_camera_;
    ...
    for (const auto& [cam_idx, pt_idx, obs] : observations) {
        ...
        ceres::CostFunction* reprojection_error =
            new ceres::AutoDiffCostFunction<ReprojectionError, 2, 6, 3>(
                new ReprojectionError(observed_keypoint, K_eigen));
...
py::tuple ba_solve_depth_mahalanobis(
    CameraPoses camera_poses,
    Point3Ds point_3ds,
    Observations3DWeighted observations,
    py::array_t<double> K,
    ConstantPoseIndex constant_pose_index,
    RelativePoseConstraints relative_pose_constraints
) {
    ...
    ceres::LossFunction* loss_function = new ceres::HuberLoss(2.0);
    for (const auto& [cam_idx, pt_idx, obs_point_cam, sqrt_information] : observations) {
        auto obs_buf = obs_point_cam.unchecked<1>();
        auto sqrt_info_buf = sqrt_information.unchecked<2>();
        Eigen::Vector3d observed_point_in_camera(obs_buf(0), obs_buf(1), obs_buf(2));
        Eigen::Matrix3d sqrt_information_eigen;
        for (ssize_t i = 0; i < 3; ++i) {
            for (ssize_t j = 0; j < 3; ++j) {
                sqrt_information_eigen(i, j) = sqrt_info_buf(i, j);
            }
        }
        ceres::CostFunction* mahalanobis_error =
            new ceres::AutoDiffCostFunction<MahalanobisPointToCameraError, 3, 6, 3>(
                new MahalanobisPointToCameraError(observed_point_in_camera, sqrt_information_eigen));
        problem.AddResidualBlock(
            mahalanobis_error,
            loss_function,
            camera_parameters.at(cam_idx).data(),
            point_parameters.at(pt_idx).data());
    }
    ...
    py::dict py_optimized_point_3ds;
    for (const auto& [pt_idx, pt_parameter] : point_parameters) {
        py_optimized_point_3ds[py::int_(pt_idx)] = py::array_t<double>({3}, pt_parameter.data());
    }
    return py::make_tuple(optimized_camera_poses, py_optimized_point_3ds);
}
