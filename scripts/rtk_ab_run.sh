...
OUT="/home/dm/rtk_ab/${LABEL}_${STAMP}"
mkdir -p "$OUT"

BRIDGE_LOG=/home/dm/workspace/tinynav/rtk_bridge.log
SIGNAL_CSV=/home/dm/workspace/tinynav/rtk_signal.csv
GW=$(ip route | awk '/^default/{print $3; exit}')
CASTER=$(grep -oE '^export TINYNAV_NTRIP_HOST=.*' /home/dm/workspace/tinynav/rtk/.ntrip.env 2>/dev/null | cut -d= -f2- | tr -d "\"'")
...

{
  ...
  echo "--- git ---";         git -C /home/dm/workspace/tinynav rev-parse --short HEAD
...
(
  echo "wall,gw_ms,caster_ms,rssi_dbm"
  while true; do
    g=$(ping -c1 -W1 "$GW" 2>/dev/null | grep -oP 'time=\K[0-9.]+' || echo NA)
    c=$(ping -c1 -W1 "$CASTER" 2>/dev/null | grep -oP 'time=\K[0-9.]+' || echo NA)
...
docker exec tinynav-dev bash -lc \
  "source /opt/ros/humble/setup.bash; cd $OUT && ros2 bag record -o ctx \
...
echo "[rtk_ab] gateway=$GW caster=$CASTER"
