...
tmux new-session -s $SESSION_NAME \; \
  ...
  select-pane -t 0 \; send-keys 'ros2 run foxglove_bridge foxglove_bridge --ros-args -p port:=8765 -p topic_whitelist:="[/camera/camera/color/image_raw_repub,/camera/camera/color/camera_info, /tf, /cmd_vel, /service/command, /service/state, slam/odometry, /planning/trajectory_path, /planning/occupied_voxels, /battery, /mapping/pointcloud_markers, /mapping/global_plan, /mapping/poi]"' C-m \; \
