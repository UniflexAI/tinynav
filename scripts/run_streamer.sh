...
tmux new-session \; \
  split-window -h \; \
  split-window -v \; \
  select-pane -t 0 \; send-keys 'ros2 run foxglove_bridge foxglove_bridge --ros-args -p port:=8765 -p topic_whitelist:="[/camera/camera/infra1/image_rect_raw_repub,/camera/camera/infra1/camera_info]"' C-m \; \
