...
def build_map_vlad_descriptors(map_path: str) -> tuple[list[int], np.ndarray, np.ndarray, TinyNavDB]:
    """Load the map's precomputed DINOv2 patch-VLAD index (matches MapNode.__init__)."""
    timestamps, db = load_map_timestamps_and_db(map_path)
    try:
        vlad_centres = db.metadata["vlad_centres"]
        map_vlad_descriptors = np.stack([db.vlad_descriptors[ts] for ts in timestamps]).astype(np.float32)
    except KeyError as exc:
        raise RuntimeError(
            "This map does not contain a DINOv2 patch VLAD relocalization index. "
            f"Rebuild it with --retrieval_backend dinov2_vlad. map_path={map_path}, missing_key={exc}"
        ) from exc
    return timestamps, vlad_centres, map_vlad_descriptors, db


def load_or_build_bow_index(map_path: str, db: TinyNavDB, timestamps: list[int]) -> BowIndex:
    bow_index_path = os.path.join(map_path, "bow_index.npz")
    if not os.path.exists(bow_index_path):
        print(f"BoW index not found at {bow_index_path}; building it from map features")
        build_bow_index(db, timestamps, bow_index_path)
    return BowIndex(bow_index_path)
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=-1.0, help="similarity cutoff for the dinov2_vlad backend (ignored by superpoint_bow)")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="skip writing per-sample query/retrieved/pnp_match PNGs (cv2.imwrite PNG compression is CPU-heavy); jsonl + sample.json metadata are still written",
    )
    ...
    if args.retrieval_backend == "dinov2_vlad":
        ...
        map_ts, vlad_centres, map_vlad_descriptors, map_db = build_map_vlad_descriptors(args.map_path)
    else:
        map_ts, map_db = load_map_timestamps_and_db(args.map_path)
        bow_index = load_or_build_bow_index(args.map_path, map_db, map_ts)
    ...
    with open(args.out_jsonl, "w", encoding="utf-8") as f, open(index_path, "w", encoding="utf-8") as index_f:
        for ts_ns, infra1 in iter_infra1_images(args.bag_path, args.topic):
            ...
            if args.retrieval_backend == "dinov2_vlad":
                ...
                query_vlad = compute_vlad(query_patch_tokens, vlad_centres)
                hits = find_loop(query_vlad, map_vlad_descriptors, args.threshold, args.topk)
                hits = [(int(map_ts[idx]), float(sim)) for idx, sim in hits]
            else:
                hits = bow_index.query(query_features, args.topk)

            retrieved = [{"timestamp_ns": ts, "similarity": sim} for ts, sim in hits]
            pnp_success = False
            inlier_count = 0
            inlier_ratio = 0.0
            pnp_match_path = ""
            ...
            if len(point_3d_in_world_list) > 0:
                ...
                if len(pts3d) >= args.pnp_min_matches:
                    ...
                    if best_match_vis is not None and not args.skip_review_images:
            ...
            if not args.skip_review_images:
                cv2.imwrite(query_path, infra1)
            retrieved_ts = []
            similarities = []
            if args.save_debug_dir and not args.skip_review_images:
