...
def depth_to_points(
    depth: np.ndarray, K: np.ndarray, pixel_step: int, max_depth: float
) -> tuple[np.ndarray, dict[str, int]]:
    h, w = depth.shape
    fx, fy = K[0, 0], K[1, 1]
    cx, cy = K[0, 2], K[1, 2]
    points = []
    stats = {"sampled": 0, "valid_depth": 0}
    for v in range(0, h, pixel_step):
        for u in range(0, w, pixel_step):
            ...
            # print("fx, fy, cx, cy, x, y, z is ", fx, fy, cx, cy, x, y, z)

            points.append((x, y, z))
    if not points:
        return np.empty((0, 3), dtype=np.float32), stats
    return np.asarray(points, dtype=np.float32), stats
...
class LocalPointCloudPublisher(Node):
    def __init__(self, args: argparse.Namespace):
        super().__init__("local_pointcloud_publisher")
        self.args = args
        ...
        self.local_cloud_buffer: deque[tuple[np.ndarray, np.ndarray, np.ndarray]] = (
            deque()
        )
        self.path_msg = Path()
        ...
        # self.debug_sensor_cloud_pub = self.create_publisher(
        #    PointCloud2, args.debug_sensor_output_topic, 10
        # )
        ...
        self.get_logger().info(
            f"Publishing local cloud on {args.output_topic} from {args.depth_topic} + {args.pose_topic} + {self.image_topic} ({args.image_mode})"
        )
    ...
    # self.get_logger().info(
    #    "Status: "
    #    f"pose_seen={self._saw_pose}, depth_seen={self._saw_depth}, gray_seen={self._saw_gray}, "
    #    f"depth_k={self.K is not None}, tf_seen={self._saw_tf}, tf_static_seen={self._saw_tf_static}, "
    #    f"imu_depth_tf={self.T_i_depth is not None}, sync_count={self._sync_count}"
    # )
    # if self.T_i_depth is not None:
    #    self.get_logger().info(
    #        "Current imu->depth translation: "
    #        f"{np.round(self.T_i_depth[:3, 3], 4).tolist()}"
    #    )
    ...
    def prune_buffer(self, current_position: np.ndarray) -> None:
        kept = deque()
        removed = False
        for sensor_position, cloud, colors in self.local_cloud_buffer:
            if (
                np.linalg.norm(sensor_position - current_position)
                <= self.args.local_radius
            ):
                kept.append((sensor_position, cloud, colors))
            else:
                removed = True
        self.local_cloud_buffer = kept
        if removed:
            self.rebuild_merged_cache()

    def rebuild_merged_cache(self) -> None:
        if not self.local_cloud_buffer:
            self.merged_cloud_cache = np.empty((0, 3), dtype=np.float32)
            self.merged_color_cache = np.empty((0,), dtype=np.uint32)
            return
        self.merged_cloud_cache = np.vstack(
            [cloud for _, cloud, _ in self.local_cloud_buffer]
        ).astype(np.float32, copy=False)
        self.merged_color_cache = np.concatenate(
            [colors for _, _, colors in self.local_cloud_buffer]
    ...
    def publish_xyz_cloud(self, points: np.ndarray, stamp, frame_id: str) -> None:
        header = Header()
        header.stamp = stamp
        header.frame_id = frame_id
        # self.debug_sensor_cloud_pub.publish(
        #    pc2.create_cloud_xyz32(header, points.tolist())
        # )
    ...
    def sync_callback(
        ...
        convert_ms = (time.perf_counter() - stage_time) * 1000.0

        raw_backproject_ms = 0.0
        # self.publish_xyz_cloud(
        #    raw_sensor_cloud, depth_msg.header.stamp, depth_msg.header.frame_id
        # )
        # if self._projection_debug_counter % 20 == 0:
        #    self.get_logger().info(
        #        f"Raw depth backprojection stats: {raw_stats}, depth_frame={depth_msg.header.frame_id}"
        #    )
        ...
        pose_transform_ms = (time.perf_counter() - stage_time) * 1000.0
        ...
        image_project_ms = (time.perf_counter() - stage_time) * 1000.0
        self._projection_debug_counter += 1
        ...
        keep_mask = crop_mask(cloud_world, current_position, self.args.local_radius)
        cloud_world = cloud_world[keep_mask]
        cloud_colors = cloud_colors[keep_mask]
        world_transform_ms = (time.perf_counter() - stage_time) * 1000.0
        ...
        if self.should_add_keyframe(T_world_camera):
            ...
            self.local_cloud_buffer.append(
        ...
        keyframe_ms = (time.perf_counter() - stage_time) * 1000.0
        ...
        stage_time = time.perf_counter()
        keep_mask = crop_mask(
            self.merged_cloud_cache, current_position, self.args.local_radius
        )
        merged_cloud = self.merged_cloud_cache[keep_mask]
        merged_colors = self.merged_color_cache[keep_mask]
        merge_downsample_ms = (time.perf_counter() - stage_time) * 1000.0
        ...
        publish_ms = (time.perf_counter() - stage_time) * 1000.0
        if not self._published_once:
            self._published_once = True
            self.get_logger().info(
                f"Published first {self.args.image_mode} local cloud with {merged_cloud.shape[0]} points in frame {pose_msg.header.frame_id or 'world'}."
            )
        elapsed_ms = (time.perf_counter() - start_time) * 1000.0
        # self.get_logger().info(
        #    "Sync-to-publish latency: "
        #    f"{elapsed_ms:.2f} ms for {merged_cloud.shape[0]} points. "
        #    f"stages(ms): convert={convert_ms:.2f}, raw_backproject={raw_backproject_ms:.2f}, "
        #    f"pose_tf={pose_transform_ms:.2f}, image_project={image_project_ms:.2f}, "
        #    f"world_tf={world_transform_ms:.2f}, keyframe={keyframe_ms:.2f}, "
        #    f"merge_downsample={merge_downsample_ms:.2f}, publish={publish_ms:.2f}"
        # )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Project SLAM depth into a local world-frame point cloud."
    )
    ...
    parser.add_argument("--output-topic", default="/mapping/local_depth_cloud")
    parser.add_argument("--path-topic", default="/mapping/trajectory_path")
...
def main(args=None) -> None:
    ...
    node = LocalPointCloudPublisher(parsed_args)
    try:
        rclpy.spin(node)
