...
def _load_normalized_vlad_embeddings(map_path: Path, timestamps: list[int]) -> np.ndarray:
    db = TinyNavDB(str(map_path), is_scratch=False)
    embeddings: list[np.ndarray] = []
    try:
        for timestamp in timestamps:
            embedding = np.asarray(db.vlad_descriptors[int(timestamp)], dtype=np.float32).reshape(-1)
            norm = float(np.linalg.norm(embedding))
            if norm <= 1e-8:
                raise ValueError(f"VLAD descriptor for timestamp {timestamp} in {map_path} has near-zero norm")
            embeddings.append(embedding / norm)
    finally:
        db.close()
    if not embeddings:
        raise RuntimeError(f"No VLAD descriptors found in {map_path}")
    return np.stack(embeddings, axis=0)
...
def _load_embeddings(
    ...
    if args.descriptor_backend == "vlad":
        return _load_normalized_vlad_embeddings(map_path, timestamps)
...
def main() -> None:
    ...
    parser.add_argument(
        "--descriptor-backend",
        choices=["stored", "vlad", "anyloc"],
        default="stored",
        help=(
            "Descriptor source. 'stored' uses embeddings.db (DINOv2 global) from each map. "
            "'vlad' uses vlad_descriptors.db (DINOv2 patch VLAD) from each map. "
